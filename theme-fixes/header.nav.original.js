...
class HeaderNav extends HTMLElement {
  connectedCallback() {
    ...
    document.addEventListener(EVENTS.headerDrawerClosed, (evt) => (this.hasActiveDrawers = false), {
     ...
     // SAMI Close compressed desktop nav on scroll down
  this.lastScrollTop = 0;
  window.addEventListener('scroll', () => {
    const currentScroll = window.scrollY;

    if (currentScroll > this.lastScrollTop && currentScroll > 50) {
      console.log('Scrolling down, closing desktop compressed nav');

      // Find the compressed nav trigger button
      const compressedTrigger = document.querySelector('.site-nav__compress-menu');

      // Find the compressed nav menu
      const compressedNav = document.querySelector('[data-type="nav"]');

      // Close it if it's active
      if (compressedNav && compressedNav.classList.contains('is-active')) {
        compressedTrigger.classList.remove('is-active');
        compressedNav.classList.remove('is-active');
      }
    }
